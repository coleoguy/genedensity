...
run.analysis <- function(rsq, repeats, clade, response = "rsq", 
  ...
  # convert model.sel to a plain data frame for export
  models.df <- as.data.frame(models)
  models.df$model.rank <- seq_len(nrow(models.df))
  models.df$dataset <- clade
  models.df$response <- response
  # move metadata columns to the front
  meta.cols <- c("dataset", "response", "model.rank")
  models.df <- models.df[, c(meta.cols, 
                              setdiff(colnames(models.df), meta.cols))]
...
# rbind for data frames with mismatched columns (e.g., Sauropsida has no
# interaction terms)
rbind.fill <- function(df.list) {
  all.cols <- unique(unlist(lapply(df.list, colnames)))
  aligned <- lapply(df.list, function(df) {
    missing <- setdiff(all.cols, colnames(df))
    df[missing] <- NA
    df[, all.cols]
  })
...
for (response in responses) {
  ...
  cur.models <- rbind.fill(lapply(main.results, function(x) x$models.df))
  all.models.df[[length(all.models.df) + 1]] <- cur.models
...
all.models.df <- rbind.fill(all.models.df)
write.csv(all.models.df, "../../../results/all-models-confidence-set.csv", row.names = FALSE)
