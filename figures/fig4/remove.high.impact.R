...
highinf.df <- keep.sig(highinf.df)
...
highinf.df <- add.aesthetics(highinf.df)
...
for (cl in clades) {
  ...
  # union of predictors, sorted (so y-axis is shared between left and right)
  all.models <- sort(unique(c(m$model, h$model)))
  ...
  n.dropped <- if (nrow(h) > 0 && "num.dropped" %in% colnames(h)) {
    h$num.dropped[1] } else "?"

  rng <- range(c(m$lower.flip, m$upper.flip, h$lower.flip, h$upper.flip), 
  ...
  title.r <- paste0("High-influence removed: ", cl, 
                    "  (", n.dropped, " species dropped)")
  draw.panel(h, title.r, rng, all.models, all.ylabs, 
