...
dat <- na.omit(final.results[, c("species", "cv", "k2p.median", "clade")])
...
# graph
ggplot(dat, aes(x = k2p.median, y = cv, color = clade)) +
  ...
  xlim(c(5, 30)) +
  labs(title = bquote("Coefficient of Variation vs Median Divergence"), 
...
ggsave(filename = paste0("cv_k2pmedian_scatter_", vert.invert, ".jpg"), 
       plot = last_plot(), 
       width = 7680, 
       height = 4320, 
       units = "px", 
