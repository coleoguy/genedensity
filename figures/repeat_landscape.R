...
for (file in files) {
  ...
  repeat.cats <- sapply(colnames(divsum.table2), 
  ...
  results.table0 <- divsum.table2[-c(as.numeric(unlist(non.unique.indices)))]
  # rename table
  colnames(results.table0) <- repeat.cats[-c(as.numeric(unlist(non.unique.indices)))]
  # add summed columns to table
  results.table <- data.frame(results.table0, non.unique.sum)
  ...
  results.table1.5 <- results.table[c(top.4.cat.ind, div.ind, unknown.ind)]
  results.table2 <- data.frame(results.table1.5, other)
  # grouped by columns
  divergence <- rep(results.table2$Div, length(colnames(results.table2))-1)
  sequence.lengths_bp <- as.numeric(unlist(results.table2[-c(1)]))
  species.under <- gsub("_summary\\.divsum$", "", file)
  species.lower <- gsub("_", " ", species.under)
  species <- gsub("^(\\w)(.*)", "\\U\\1\\L\\2", species.lower, perl = TRUE)
  gnsz_bp <- unique(combined.results$asmbly.size_bp[combined.results$species == species])
  percent.of.genome <- sequence.lengths_bp / gnsz_bp * 100
  repeat.group <- as.character(
    sapply(colnames(results.table2)[colnames(results.table2) != "Div"], 
           function(rowname) rep(rowname, length(results.table2$Div))))
  ...
  rsq <- unique(combined.results$speciesRsquared[combined.results$species == species])
