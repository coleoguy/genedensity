...
dat <- na.omit(dat[, c("species", "clade", "rsq", "est.gnsz.Mbp", "meansize")])
dat$normrsq <- dat$rsq / dat$meansize
...
# graph
ggplot(dat, aes(x = meansize, y = rsq, color = clade)) +
  coord_trans(x = "log10") +
  ...
  labs(title = bquote(italic(R)^2 ~ "vs Mean Chromosome Size"), 
       subtitle = bquote(italic(β) == .(slope) * "," ~~ italic(β) ~ italic(p) * "-value" == .(slope.pval)),
       x = bquote("Mean Chromosome Size (Mb)"), 
       y = bquote(italic(R)^2)) +
  xlim(c(min(meansize), 550))
ggsave(filename = paste0("rsq_chromsize_scatter.jpg"), 
       plot = last_plot(), 
       width = 7680, 
       height = 4320, 
       units = "px", 
